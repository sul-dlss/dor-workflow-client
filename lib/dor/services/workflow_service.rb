...
module Dor
  ...
  class WorkflowService
    ...
    # @param [String, RestClient::Resource] url or configured RestClient::Resource that points to the workflow service
    ...
    # @option opts [Integer] :timeout number of seconds for RestClient timeout, default: 180
    # @option opts [Boolean] :cache_enabled true to enable caching, default: false, unless :cache supplied
    # @option opts [ActiveSupport::Cache] :cache configured cache object, default ActiveSupport::Cache::MemoryStore. Defaults :cache_enabled to true.
    # @option opts [Proc]    :handler code triggered by RestClient exception, receives the exception object, attempt_number and total_delay.  See retries gem.
    ...
    def initialize(url, opts = {}) # rubocop:disable Metrics/CyclomaticComplexity, Metrics/PerceivedComplexity
      ...
      @resource            = url.is_a?(RestClient::Resource) ? url : RestClient::Resource.new(url, params)
      ...
      @exceptions_to_catch = opts[:exceptions_to_catch] ? Array(opts[:exceptions_to_catch]) : [RestClient::Exception]
      @logger              = opts[:logger]  || Logger.new('workflow_service.log', 'weekly')
      @cache               = opts[:cache]   || ActiveSupport::Cache::MemoryStore.new
      ...
      @cache_enabled = opts.fetch(:cache_enabled, opts[:cache] ? true : false)
    end

  ...
  describe '#create_workflow_by_name' do
    ...
    it 'logs an error and retry upon a targeted Faraday exception' do
      expect(mock_logger).to receive(:warn)
        .with("retrying connection (1 tries) to #{request_url}: (Faraday::RetriableResponse)  500")
      expect(mock_logger).to receive(:warn)
        .with("retrying connection (2 tries) to #{request_url}: (Faraday::RetriableResponse)  500")
